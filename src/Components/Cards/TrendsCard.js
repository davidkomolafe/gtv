...
const TrendsCard = ({ movie }) => {
  return (
    <div className="relative h-trend rounded-3xl w-30 flex flex-col hover:transition hover:scale-1 max-[450px]:w-35">
      <Link to={`/movies/${movie.name}`} className="w-full h-full">
        ...
        />
        <div className="absolute w-full top-0 left-0 p-8 text-3xl flex justify-between">
